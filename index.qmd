...
olympics_data <- readr::read_csv(url, locale = locale(encoding = "UTF-8"))
olympics_data
```
...
We do not apply this previous code chunk here, in order to not publish the API keys. We will rather use the translated tweets saved in csv-file in GitHub. They have been translated with the excact same code as above using the DeepL API.
:::

```{r warning=FALSE, message=FALSE}
url <- "https://raw.githubusercontent.com/YannikPeters/DQ_Tool_TextPreprocessing/main/data/olympics_data_de_deepl.csv"
olympics_data_de_deepl <- readr::read_csv(url, locale = locale(encoding = "UTF-8"))
...
deepl_translation_de <- as.character(olympics_data_de_deepl$tweet_text)
...
With a mean cosine similarity of 0.77 on a scale from 0 to 1, the translations from Google Translate and DeepL are indeed quite similar, but still there are some differences. We can apply the same process for the French postings.
...
#again this Code chunk won't be applied here in order to not publish API keys (see call-out box above)
...
```{r message=FALSE, warning=FALSE}
url_1 <- "https://raw.githubusercontent.com/YannikPeters/DQ_Tool_TextPreprocessing/main/data/olympics_data_fr_deepl.csv"
olympics_data_fr_deepl <- readr::read_csv(url_1, locale = locale(encoding = "UTF-8"))
```
...
google_translation_fr <- as.character(olympics_data_fr_google$tweet_text)
deepl_translation_fr <- as.character(olympics_data_fr_deepl$tweet_text)
